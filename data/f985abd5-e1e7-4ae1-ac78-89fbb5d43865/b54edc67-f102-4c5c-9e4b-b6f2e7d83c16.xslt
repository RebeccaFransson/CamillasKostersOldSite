...
<xsl:stylesheet
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="fixedHref" >
        <xsl:call-template name="fixRssHref">
          <xsl:with-param name="niceUrl">
            <xsl:value-of select="umbraco.library:NiceUrl($currentPage/ancestor-or-self::node [@nodeTypeAlias = 'Blog']/@id)" />
          </xsl:with-param>
      </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:if test="$currentPage/@nodeTypeAlias = 'BlogPost' and string($currentPage/data [@alias = 'closeComments']) != '1'">
