// HELPERS
function openModal(url, name, height, width) {
    var dialog = jQuery('#umbModalBox').jqm({
        onShow: function(h) {
            var umbModal = jQuery(h.w);
            var umbModalContent = jQuery("iframe", umbModal);

            umbModalContent.width(width);
            umbModalContent.height(height - 30);

            jQuery("#umbModalBoxHeader").html(name);

            if (height > 0) umbModal.height(height);

            if (width > 0) {
                umbModal.width(width);
                umbModal.css("left", ((jQuery(document).width() - width) / 2) + "px");
            }

            umbModalContent.html('').attr('src', createUniqueUrl(url));
...
function createUniqueUrl(url) {
    ...
    var rnd = (z - (parseInt(z / 1000, 10) * 1000)) / 10
...
function fitModalToPageSize(modalHeight, width) {
